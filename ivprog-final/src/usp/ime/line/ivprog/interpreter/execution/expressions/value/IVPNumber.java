...
public class IVPNumber extends IVPValue {
	 ...
	 * The method resolves the primitive type by itself.
	 * @param value
	 * @param context
	 * @param factory
	 * @return
	...
	public IVPNumber remainder(IVPNumber number, Context context, DataFactory factory){
		IVPNumber result = factory.createIVPNumber();
		BigDecimal n = context.getBigDecimal(getUniqueID()).remainder(context.getBigDecimal(number.getUniqueID()));
		context.addBigDecimal(result.getUniqueID(), n);
		result.updateValue(context, n);
		return result;
	}
